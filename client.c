...
#include <stdlib.h>      // strtol(), memset()
#include <string.h>      // strcmp(), strcat() ... (string related)
...
void setupAndConnect(struct sockaddr_in *serverAddr, struct hostent *host,
                     int socketFd, long port);
...
void interruptHandler(int sig_unused) {
  sig_unused = sig_unused;  // to make complier happy
